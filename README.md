...
## 🕹Installing/Using🕹
Installing it is fairly easy, because there is no installation. Just download the repo, configure it and you're good to go. Configuring is done inside of the config.py file, where you can set your gamemode, storage type, etc. After you configured your csgoMELON instance, copy the gsi.cfg file into your Counterstrike: Global Offensive directory into csgo/cfg. The full path will probably be something along those lines: C:\Program Files (x86)\Steam\steamapps\common\Counter-Strike Global Offensive\csgo\cfg
After you've set everything up just execute manage.py and everything will work automatically.

## ⛔️Important notes⛔
The player using the GSI has to be an observer in your CS:GO match. That's just another name for a spectator. So if you're executing the csgoMELON you can't participate in the game. But you could join the lobby with a second account on a different machine and execute csgoMELON there.
